...
struct SidebarView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            HStack {
                Button {
                    NotificationCenter.default.post(
                        name: store.browserMode == .skills ? .openAddSkillSheet : .openAddProfileSheet,
                        object: nil
                    )
                } label: {
                    Label(store.browserMode == .skills ? "New Skill" : "New Profile", systemImage: "plus.circle.fill")
                        .fontWeight(.semibold)
            ...
            if store.browserMode == .profiles {
                List {
                    Section {
                        Label("All Profiles", systemImage: "square.stack.3d.up.fill")
